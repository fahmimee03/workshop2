...
def extract_desired_text(filename):
    ...
    # Save all extracted details to a single CSV file
    output_file_path = os.path.join(app.config['OUTPUT_FOLDER'], 'extracted_data.csv')
    with open(output_file_path, 'w') as f:
        # Write Monthly Charges
        f.write("Months,Charges\n")
        for month, charge in zip(months, charges):
            f.write(f"{month},{charge}\n")
        
        # Add a separator for Detailed Charges
        f.write("\nDetailed Charges\n")
        f.write(detailed_charges_text.replace("\n", "\r\n"))  # Ensure newlines are handled in CSV format
